...
void print_mixer_mode_options(Stream &stream) {
  stream.println("[CONSOLE] Mixer mode options:");
  stream.println("[CONSOLE]   0: Stereo");
  stream.println("[CONSOLE]   1: Stereo Inverse");
  stream.println("[CONSOLE]   2: Mono");
  stream.println("[CONSOLE]   3: Right");
  stream.println("[CONSOLE]   4: Left");
}

bool apply_mixer_mode(Tas58xx &amp, const String &value_text, Stream &stream) {
  int mode = 0;
  if (!parse_int_range(value_text, 0, 4, &mode)) {
    stream.println("[CONSOLE] Mixer mode out of range. Use mix:0..4");
    return false;
  }

  if (!amp.setMixerMode(static_cast<Tas58xx::MixerMode>(mode))) {
    stream.println("[CONSOLE] Failed to set mixer mode");
    return false;
  }

  stream.printf("[CONSOLE] Mixer mode set to %d\n", mode);
  return true;
}
